...
export async function dailyCron() {
    ...
    console.log( "Running daily mail job at", new Date().toLocaleString("en-US", { day: "2-digit", month: "short", year: "numeric", hour: "2-digit", minute: "2-digit", second: "2-digit", hour12: false,}))
    await mongoConnect()
    ...
    try {
        ...
        const emailPromises = todaySlot.map(async (slot)=>{
            ...
            console.log(`Email sent to ${slot.email} for event: ${slot.name}`);
            if (slot.type=='birthday'){
